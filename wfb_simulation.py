...
class Return_wfb_simulation:
    def __init__(self, loser, round, unit1_size, unit2_size):
         self.loser = loser
         self.round = round
...
def wfb_simulation(unit1, unit2, runs, filename = 'results'):
    ...
    with open(filename + ".csv", "w") as output:
        ...
        outputwriter.writerow(["Loser", "Rounds", "Final_size_" + str(unit1.name), "Final_size_" + str(unit2.name)])
        ...
        for i in range(0, runs):
            ...
            if c_result.loser == None:
                outcome = Return_wfb_simulation(loser = "Both", round = c_result.round,
                                                unit1_size = unit1_copy.models,
                                                unit2_size = unit2_copy.models)
            else:
                outcome = Return_wfb_simulation(loser=c_result.loser, round=c_result.round,
                                                unit1_size=unit1_copy.models,
                                                unit2_size=unit2_copy.models)

            outputwriter.writerow([outcome.loser.name, outcome.round,
